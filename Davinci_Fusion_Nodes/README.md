...
> [!IMPORTANT]
> Both videos must have the **same frame count and frame rate**. The convergence control video is a 1:1 frame-for-frame map of the SBS footage. Resolution does **not** need to match — the control video is intentionally rendered at a tiny 64×64 size, since the `Probe` node only samples its center pixel.
...
| `Probe` values look compressed (never reach 0.0 or 1.0) | The control video is encoded as **full-range**. If Resolve interprets it as video levels, right-click the clip in the Media Pool → **Clip Attributes → Video → Data Levels** and set it to **Full**. |
